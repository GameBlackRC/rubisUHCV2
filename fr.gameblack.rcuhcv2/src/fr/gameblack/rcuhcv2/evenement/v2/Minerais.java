...
public class Minerais {
	...
	public static void InteractMinerais(Joueur joueur, Material minerais, Location bloc_loc) {
		...
		if(minerais == Material.GLOWSTONE) {
			...
			else if(joueur.isUseRubisBleu()) {
				
				joueur.getPlayer().sendMessage("Vous avez déjà récupérer un rubis bleu");
				
			}
			else {
				
				joueur.setUseRubisBleu(true);
				Bukkit.getWorld("world").getBlockAt(bloc_loc).setType(Material.AIR);
	...
	public static void InteractItemRubis(Joueur joueur, Main main) {
		...
		joueur.getPlayer().getInventory().remove(new ItemStack(Material.NETHER_BRICK_ITEM, 1));
        ...
        if(nb == 1) {
        	...
        	ItemCD cycle = new ItemCD(main, joueur, "rubis_force", 60, joueur, null, null, 0, null);
        	...
        	joueur.addResi(0.02);
        	ItemCD cycle = new ItemCD(main, joueur, "rubis_resi", 60, joueur, null, null, 0, null);
	        cycle.runTaskTimer(main, 0, 20);
        	...
        	ItemCD cycle = new ItemCD(main, joueur, "rubis_speed", 60, joueur, null, null, 0, null);
	        cycle.runTaskTimer(main, 0, 20);
	...
	public static void InteractItemRubisBleu(Joueur joueur, Main main) {
		...
		joueur.getPlayer().getInventory().remove(new ItemStack(Material.QUARTZ, 1));
	...
	public static void InteractItemTopaze(Joueur joueur, Main main) {
		...
		joueur.getPlayer().getInventory().remove(new ItemStack(Material.GLOWSTONE_DUST, 1));
